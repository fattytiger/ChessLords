...
cc.Class({
    ...
    judgeRegion() {
        ...
        this.maplocation.leftCityJudgeOthercity('archer')
    },
    restoreRegion() {
        this.maplocation.updateSoliderLocation()
        for (let i = 1; i < 9; i++) {
            cc.find(`Canvas/background/LeftCity/archer/fightRegion/region${i}`).active = true
        }
    },
    ...
    onLoad() {
        ...
        this.node.on('foobar',function(){
            console.log('left archer recive')
        })

        cc.find(`Canvas/background/LeftCity/knight`).getComponent('leftKnight').broArcher = this
        cc.find('Canvas/background/LeftCity/troop').getComponent('leftTroop').broArcher = this
    },
